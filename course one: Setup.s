...
总结：
    ...
    2、开机后显示的命令行是怎么回事？
       答：当系统开机执行上述第一步后，会在初始化结束时启动shell程序，此时shell程序会调用scanf()函数在显示界面打印出类似“[root@izbp11zw34sma1jftiozr0z ~]#_” 等待用户输入命令，而命令属于可执行文件，
       此时会根据"if(!fork()){exec(cmd)}"申请CPU，再根据output.c进行编译执行，最后输出到显示器上。
